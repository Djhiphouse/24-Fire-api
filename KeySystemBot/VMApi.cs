...
namespace KeySystemBot
{
	...
	public class SellixKeySystem
	{
		...
		public bool IsKeyUsebyAuthor(String Vmid, String User)
		{
			try
			{
			...
			/*
			 * string[] users = File.ReadAllLines("Vmid.txt");
				foreach (string user in users)
				{
					string userData = user;
					if (userData.Split(':')[0] == Vmid && userData.Split(':')[1] == User)
					{
						Console.WriteLine("Benutzername ist bereits vergeben");
					//	return false;
					}
				...
				using (StreamWriter writer = File.AppendText("Vmid.txt"))
				{
					writer.WriteLine(Vmid + ":" + User);
				}

				Console.WriteLine("Erfolgreich Gecheckt");
			*/
		}
	}
	internal class VMApi
	{
		...
		String SellixKey = "Sellix API KEY";
		String HostResellKey = "24Fire API";
