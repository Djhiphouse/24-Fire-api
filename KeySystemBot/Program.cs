...
namespace KeySystemBot
{
	internal class Program
	{
		...
		public String CustomerRole = "Customer";
		...
		private async Task<Task> CommandHandler(SocketMessage message)
		{
			try
			{
				...
				var messagee = message as SocketUserMessage;
				var context = new SocketCommandContext(_client, messagee);



				if (message.Content.StartsWith("!addperms") && UserPerms.Contains(message.Author.Id))
				{
					...
					try
					{
						...
						val.WithTitle("ShopName Permission");
						val.AddField("Added Permission to UserID::", Args[0]);
						val.AddField("Added By: ", message.Author);
					...
					{
						EmbedBuilder val = new EmbedBuilder();
						val.WithTitle("ShopName Permission Failed!");
				...
				if (message.Content.StartsWith("!pay"))
				{
					...
					if (KeySystem.SellixCheckKey(message.Content.Replace("!pay ", "")))
					{
						EmbedBuilder val = new EmbedBuilder();
					...
					else
					{
						...
						val.WithColor(Color.Green);
						await message.Channel.SendMessageAsync("", false, val.Build());
						var role = context.Guild.Roles.FirstOrDefault(x => x.Name == CustomerRole);
						IUser userr = message.Author;
					...
					if (Product == "63d6f67269074")
					{
						...
						SellixKeySystem sellixKeySystem = new SellixKeySystem();
						//	sellixKeySystem.IsKeyUsebyAuthor(Server[0], message.Author.Id.ToString());
						EmbedBuilder val = new EmbedBuilder();
						...
						// Assuming you have a SocketMessage object named "message" available
						var userr = message.Author as SocketUser;
						var dmChannel = await userr.CreateDMChannelAsync();

						// You can now send a message to the user using the dmChannel object
						await dmChannel.SendMessageAsync("**Server Created Sucessfully**\n" + "IP:" + Server[3] + "\nRoot Password: " + Server[2]);

						//await message.Author.CreateDMChannelAsync();
				...
				//	if (!user.Roles.Any(r => r.Name == CustomerRole)) return Task.CompletedTask;

				if (message.Content.StartsWith("!Create") && message.Author.Id == 1067545233811849227 || UserPerms.Contains(message.Author.Id))
				{
					...
					try
					{
						Console.WriteLine("Startetd");
						API.CreateServer(Server[Int32.Parse(Args[1])], Int32.Parse(Args[2]), Int32.Parse(Args[3]), Int32.Parse(Args[4]));
				...
				if (message.Content.StartsWith("!setrdns"))
				{
					...
					try
					{
						...
						if (sellixKeySystem.IsKeyUsebyAuthor(Args[1], message.Author.Id.ToString()))
						{
							Console.WriteLine("Failed key is ued by other");
							return Task.CompletedTask;
						}
				...
				if (message.Content.StartsWith("!resetpassword"))
				{
					...
					try
					{
						...
						String Data = API.ResetPasword(Args[1]);
						EmbedBuilder val = new EmbedBuilder();
